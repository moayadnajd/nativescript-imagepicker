...
export class SelectedAsset extends imageAssetModule.ImageAsset {
  ...
  private decodeUri(
    ...
    let bitmap = BitmapFactory.decodeStream(
  ...
  private getByteBuffer(uri: android.net.Uri): java.nio.ByteBuffer {
    ...
    try {
      ...
      let buffer: java.nio.ByteBuffer = java.nio.ByteBuffer.allocateDirect(
        length
      );
        ...
        if (requestCode === RESULT_CODE_PICKER_IMAGES) {
          if (resultCode === Activity.RESULT_OK) {
            try {
              ...
              if (clip) {
                let count = clip.getItemCount() + self.count;
                if (count <= 5) {
            ...
            reject(
              new Error("Image picker activity result code " + resultCode)
            );
            return;
          }
        }
