...
export function pageLoaded(args) {
  ...
  list.listViewLayout.spanCount = Math.floor(
    platform.screen.mainScreen.widthDIPs / 80
  );

  application.on("orientationChanged", function(
    e: application.OrientationChangedEventData
  ) {
...
export function imagesPageFactory(): Page {
  ...
  item.bind({
    targetProperty: "text",
    sourceProperty: "imagePicker.selection.length",
    twoWay: false,
    expression:
      "imagePicker.doneText + (imagePicker.mode === 'single' ? '' : ' (' + imagePicker.selection.length + ')')"
  });
  ...
  listView.itemTemplate =
    ...
    <Image height="78" width="78"  src="{{ $value }}"/> \
    <Label row="0" verticalAlignment="top" horizontalAlignment="right" visibility="{{ selected ? \'visible\':\'hidden\' }}" class="fa btn-gallery-info"    text="&#xf00c;"/> \
    </GridLayout>';
