...
export class ImagePicker extends data_observable.Observable {
  ...
  get doneText(): string {
    return this._options && this._options.doneText
      ? this._options.doneText
      : "Done";
  }

  get cancelText(): string {
    return this._options && this._options.cancelText
      ? this._options.cancelText
      : "Cancel";
  }

  get albumsText(): string {
    return this._options && this._options.albumsText
      ? this._options.albumsText
      : "Albums";
  }
  ...
  get mode(): string {
    return this._options &&
      this._options.mode &&
      this._options.mode.toLowerCase() === "single"
      ? "single"
      : "multiple";
  }
  get count(): number {
...
export class SelectedAsset extends imageAssetModule.ImageAsset {
  ...
  getImage(options?: ImageOptions): Promise<image_source.ImageSource> {
    return Promise.reject(
      new Error(
        "getImage() is not implemented in SelectedAsset. Derived classes should implement it to be fully functional."
      )
    );
  }
  ...
  getImageData(): Promise<ArrayBuffer> {
    return Promise.reject(
      new Error(
        "getImageData() is not implemented in SelectedAsset. Derived classes should implement it to be fully functional."
      )
    );
  }
}
...
export class Asset extends SelectedAsset {
  ...
  set selected(value: boolean) {
    ...
    if (value) {
      ...
      if (index < 0) {
        let imageCount =
          this.album.imagePicker.selection.length +
          this.album.imagePicker.count;
        if (imageCount <= 5) {
...
class ImagePickerPH extends ImagePicker {
  ...
  addAlbumForAssetCollection(assetCollection: PHAssetCollection): void {
    ...
    let pfAssets = PHAsset.fetchAssetsInAssetCollectionOptions(
      assetCollection,
      null
    );
  ...
  createPHImageThumbAsset(target, asset: PHAsset): void {
    PHImageManager.defaultManager().requestImageForAssetTargetSizeContentModeOptionsResultHandler(
      asset,
      this._thumbRequestSize,
      PHImageContentMode.AspectFill,
      this._thumbRequestOptions,
      function(target, uiImage, info) {
        let imageAsset = new imageAssetModule.ImageAsset(uiImage);
        imageAsset.options = {
          width:
            this._options.maxWidth && this._options.maxWidth < IMAGE_WIDTH
              ? this._options.maxWidth
              : IMAGE_WIDTH,
          height:
            this._options.maxHeight && this._options.IMAGE_HEIGHT < 80
              ? this._options.IMAGE_HEIGHT
              : IMAGE_HEIGHT,
  ...
  ): Promise<image_source.ImageSource> {
    return new Promise<image_source.ImageSource>((resolve, reject) => {
      let size: CGSize = options
        ? CGSizeMake(options.maxWidth, options.maxHeight)
        : PHImageManagerMaximumSize;
      let resizeMode = PHImageRequestOptions.alloc().init();
      let aspectRatio =
        options && options.aspectRatio && options.aspectRatio === "fill"
          ? PHImageContentMode.AspectFill
  ...
  private initialize(): void {
    ...
    let smart = PHAssetCollection.fetchAssetCollectionsWithTypeSubtypeOptions(
...
class AssetPH extends Asset {
  ...
  private _initializeOptions(options: ImageOptions): void {
    if (options) {
      this.options = {
        width:
          options.maxWidth && options.maxWidth < IMAGE_WIDTH
            ? options.maxWidth
            : IMAGE_WIDTH,
        height:
          options.maxHeight && options.maxHeight < IMAGE_HEIGHT
            ? options.maxHeight
            : IMAGE_HEIGHT,
        keepAspectRatio: true
      };
  ...
  getImage(options?: ImageOptions): Promise<image_source.ImageSource> {
    return (<ImagePickerPH>(<AlbumPH>this.album).imagePicker).createPHImage(
      this._phAsset,
      options
    );
  }
